...
public class University {
    ...
    public University(University uni1){

        this.uniName = uni1.uniName;
        this.Location = uni1.Location;
        this.rectorName = uni1.rectorName;

        
        for (int i = 0; i < departments.length; i++) {
            this.departments[i] = uni1.departments[i];
        }
    }

    public University eqUniversity(University uni){
        

        if(Arrays.equals(this.departments, uni.departments)){
            String newName = this.uniName;
            String newRector = this.rectorName;
            String newLocation = this.Location;

            String[] deps = new String[this.departments.length];

            for (int i = 0; i < this.departments.length; i++) {
                deps[i] = this.departments[i];
            }

            University temp = new University(newName, newLocation, newRector, deps);
            return temp;
